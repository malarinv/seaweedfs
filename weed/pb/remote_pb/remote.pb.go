...
type RemoteConf struct {
	...
	// local backend serves a host filesystem tree as a remote storage target.
	// Reads of bucket/path pairs resolve to $(local_path)/$(path) on disk.
	// Writes are not supported: this backend exists for read-only cache-on-read
	// migration scenarios (e.g. legacy HDD exposed via FUSE mount).
	LocalPath     string `protobuf:"bytes,80,opt,name=local_path,json=localPath,proto3" json:"local_path,omitempty"`
...
func (x *RemoteConf) GetLocalPath() string {
	if x != nil {
		return x.LocalPath
	}
	return ""
...
const file_remote_proto_rawDesc = "" +
	"\n" +
	"\fremote.proto\x12\tremote_pb\"\x89\x0f\n" +
	"\n" +
	"RemoteConf\x12\x12\n" +
	...
	"\x0econtabo_region\x18G \x01(\tR\rcontaboRegion\x12\x1d\n" +
	"\n" +
	"local_path\x18P \x01(\tR\tlocalPath\"\xff\x01\n" +
